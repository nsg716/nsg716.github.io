...
또한, 창의적인 해결책을 찾아 구현하는 것을 개발자로써 증명하겠습니다. 
...
- **(2024.1~2024.3) 스프링 학점 이수 시각화 실습 프로젝트**
  ...
  - 프로젝트 결론 :
    - 배운 점
      -  매핑에 대한 개념 
      -  JSON 활용과 상호작용 , 서버 세팅(TOMCAT)
      -  Thymeleaf 캐시에 대한 대처를 배웠다.
      -  웹페이지 구조와 구현, 디자인에 대한 이해도를 쌓았다.
      -  Thymeleaf 캐시에 대한 대처를 배웠다.
    - 아쉬운 점
      - 처음이여서 무작정 시작했지만 처음부터 설계에 집중했다면 시간이 덜 걸렸을 것 같다.
      - 데이터베이스를 활용하지 못했다. (MySQL - ERD 까지 다 했으나 JDBC를 다루지 못했다.)
      - HTML.CSS,JS 코드 부분에 시간을 80% 이상 디자인 부분에 시간을 투자한 것이 아쉽게 느껴졌다. (데이터 시각화, 가공은 별로 걸리지 않았지만 디자인에 너무 큰 시간을 투자했다.)
...
- **(2024.03.14~2024.10.31) 한이음 ICT 멘토링- 생성형AI 를 활용한 교통사고 예방 챗봇 웹 구축 팀 프로젝트**
   ...
   - 제작의도 :
     - 설계 배경 및 목표
        - 문제 해결: 교통사고 예방하기 위한 정보 제공을 목적으로 챗봇은 사용자에게 관련 정보를 제공하고 주의 사항을 알리는 데 주요한 역할을 한다.
   ...
   - 주요 기능, 기술 :
     - 동작 과정
       - 사용자 챗봇에 LLM 검색 변수를 입력하고 프롬프트를 입력하면 해당 프롬프트를 가이드라인에 따라서 AWS에 연결된 데이터베이스를 조회를 하기 된다.
   ...
   - 프로젝트 결론 :
     - 배운 점
       - 생성형 AI 프롬프트 작성법 및 데이터 전처리
       - 데이터 모델 생성과정에서 회귀분석에 대해 학습
       - AWS 데이터 베이스와 Stremalit_cloud 연결 방법
       - 생성형 AI 평가 방법
       - Ollama 모델 활용법 및 실제 구현
       - 챗봇 구현 및 제작설계서 도식 작성법
       - 깃허브 관리 방법
      - 아쉬운 점
        - 데이터 전처리를 하는데 있어서 시간을 너무 많이 사용했다는 점
        - API 섨계에 있어서 colab에서 할 수 있는 환경들이 너무 제한적이였습니다,
...
- **(2024.5~2024.6) 자산 포트폴리오 시각화 프로젝트**
   ...
   - 프로젝트 결론 :
     - 배운 점
       - 모든 기능을 세부적으로 어떻게 동작할지 기획을 먼저 하고 개발을 착수
       - 기능 단위를 분리하여 각각의 기능 단위를 빠르게 개발을 하고 3일마다 한 번씩 단위 테스트를 진행을 하여 프로젝트의 완성도를 높임
       - JDBC 연결 방법 및 스프링 동작시 DAO 클래스에 따른 패키기 분리 방법
       - Component, Entity, Service 패키지를 따로 만들어 객체지향적 프로그래밍을 적극적으로 활용
       - 깃허브를 통해 형상 관리
       - 어노테이션 활용, 시퀀스 다이어그램 작성
       - 일부이긴 하지만 RESTAPI를 사용하여 개발을 진행하였다.
     - 아쉬운 점
        - 직관적인 프로그래밍을 하다 보니 디자인 부분이 부실해졌다. (시각화 기능은 정상적으로 동작하지만 그 외 CSS 기능이 없었다.)
        - 시간이 부족하여 도입하지 못한 기능들이 많았다. (동시에 프로젝트 진행을 해서 많은 시간을 투자하기 힘들었다.)
        - JSP를 사용하지 못한 점, 스프링에서 보안에 대하여 특정한 조치가 없었고, Log의 기능을 사용하지 못한점, AOP를 사용하지 못한점 등 스프링을 100%로 활용하지 못했다.
        - 코드 리팩토링을 할 시간이 없어서 코드를 다소 지저분하게 작성을 했다. 
...
- **(2024.5~2024.6) 국내 개인 경제 상태 및 위치 데이터 분석 프로젝트**
   ...
   - 제작의도 :
     - 데이터 관계 분석에서 데이터의 특성들을 파악하여 상향식 분석을 통하여 문제를 해결하였다. 상향식 분석 방법을 선택한 이유는 주제에 대한 제약이 없었으며, 데이터로부터 문제를 도출하는 과정을 실습하는 것이 목정이였다.
     - Streamlit을 통하여 시연까지 한 이유는 발표 진행과 상관없이 데이터 분석이 궁금한 청중들을 위해 준비를 하였다.
     - 다중공선성때문에 상관관계 분석을 진행하였다.
   - Skils :
   ...
   - 프로젝트 결론 :
     - 배운 점
       - 데이터 수집에 있어서 KOSIS 공공데이터 포털을 활용하는 방법
       - 상향식 관계 분석법
       - Numpy 사용법 및 데이터 시각화 방법 (pandas,matplotlib 등을 활용) - streamlit으로 구현
       - 데이터 전처리 방법
     - 아쉬운 점 :
       - 상향식 관계 분석을 하다보니 시간을 많이 투자를 해야하며 데이터 양 자체가 많다보니 시간이 많이 소요가 되었다.
       - numpy에 전치 방법을 처음에 알았으면 시간을 줄일 수 있었다.
       - 자산 데이터가 다른 소득, 소비, 부채 데이터에 비해서 큰 부분을 차지하고 있는데 이에 대한 스케일링이 부족했다.
       - 일부 자료가 최신 데이터가 아닌 것도 있어서 사용하지 못한 데이터도 있다.
       - 데이터 안에서 다른 주제를 잡고 여러가지 분석을 할 수 있었지만 이 부분은 확장성으로 남겨두고 프로젝트를 마무리를 한것
...
- **(2024.09.28~2024.11.04) 제 3회 유통데이터 활용 경진대회 - LLM과 사용자 페르소나(User Persona)를 활용하여 맞춤형 소비자 상품 추천 시스템 및 HS Code – KAN 코드 매칭 프로젝트(우수상🏆 , 대한상공회의소 회장상)**
   ...
   - 제작의도 :
     - 사용자 페르소나를 활용하여 LLM 기반 추천 시스템을 구축을 한다. 이를 통해 사용자에게 더 적합한 제품을 추천하고, 사용자에게는 개인화된 경험을 제공할 수 있다.
   ...
   - 프로젝트 결론 :
     - 배운 점 
        - 모델 비교: Ollama llama3-8b-chat 모델을 사용하면서, llama2 모델에 비해 더 다양한 답변을 생성할 수 있다는 것을 알 수 있었다.
        - LLM-Tag 생성 및 RAG 활용: 시스템 프롬프트와 사용자의 조건에 따라 LLM-Tag를 만들고, 데이터를 RAG(우선 순위 기반 검색)로 처리한 후 KAN 코드와 매칭하는 방법
        - FAISS와 BERT를 통해 임베딩 모델을 자유롭게 비교해볼 수 있었다.
        - PCA를 통해 임베딩 데이터의 차원 축소를 하여 시각화를 진행하였다.
     - 아쉬운 점 
       -   LLM을 사용하여 HS Code에 따른 해당국가의 무역 정보를 안내를 하는 방법과, OCR과 LLN을 조합하여 해외제품 인식 방법
